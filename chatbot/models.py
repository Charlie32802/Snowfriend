...
class Message(models.Model):
    ...
    is_media_message = models.BooleanField(
        default=False, help_text="True if this message contains media content"
    )
...
class UserMemory(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="long_term_memory"
    )
    ...
    typical_session_length = models.IntegerField(
        default=0, help_text="Average number of messages per conversation"
    )
    ...
    def get_memory_summary(self) -> str:
        ...
        if self.mentioned_topics:
            sorted_topics = sorted(
                self.mentioned_topics.items(), key=lambda x: x[1], reverse=True
            )[:5]
            topics_str = ", ".join(
                [f"{topic} ({count}x)" for topic, count in sorted_topics]
            )
        ...
        if self.common_emotions:
            sorted_emotions = sorted(
                self.common_emotions.items(), key=lambda x: x[1], reverse=True
            )[:3]
            emotions_str = ", ".join(
                [f"{emotion} ({count}x)" for emotion, count in sorted_emotions]
            )
    ...
    def get_top_topics(self, limit=5) -> list:
        ...
        # ✅ Filter out non-integer values before sorting
        valid_topics = {
            topic: count
            for topic, count in self.mentioned_topics.items()
    ...
    def has_mentioned(self, topic: str) -> bool:
        ...
        return (
            topic in self.mentioned_topics
            or topic in self.mentioned_people
            or topic in self.mentioned_places
            or topic in self.mentioned_activities
        )
...
class MessageLimit(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="message_limit"
    )
    ...
    def use_message(self):
        ...
        if self.messages_remaining > 0:
            ...
            if self.messages_remaining == 0:
                ...
                # Add 10 second buffer to account for response/display delay
    ...
    def should_notify_half(self):
        ...
        return (
            self.messages_remaining <= half
            and self.messages_remaining > 3
            and not self.notified_half
        )
