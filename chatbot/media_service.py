...
class MediaService:
    ...
    # 🚨 HARD LIMIT: Absolute maximum results (buffer for pagination)
    MAX_RESULTS = 15
    ...
    def search_youtube_videos(
        self, query: str, max_results: int = 3, safe_search: str = "moderate"
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            print(
                f"✅ Found {len(results)} YouTube videos for: {query} (max: {self.MAX_RESULTS})"
            )
            return results
    ...
    def search_images(
        self, query: str, per_page: int = 3, orientation: str = "landscape"
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            print(
                f"✅ Found {len(results)} Pexels images for: {query} (max: {self.MAX_RESULTS})"
            )
            return results
    ...
    def search_media(
        self, query: str, media_type: str = "video", count: int = 3
    ) -> Dict:
        """
        ...
        if len(results) > self.MAX_RESULTS:
            print(
                f"⚠️ CRITICAL: Got {len(results)} results, truncating to {self.MAX_RESULTS}"
            )
