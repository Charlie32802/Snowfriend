...
const setupCounterTooltip = () => {
    ...
    const showTooltip = () => {
        ...
        // Update tooltip message count dynamically - use 15 as default only if undefined
        const currentMessages = window.messageLimitState?.remaining !== undefined
            ? window.messageLimitState.remaining
            : 15;

        const tooltipCounter = tooltip.querySelector('#tooltipMessagesLeft');
        if (tooltipCounter) {
            // ✅ FIX: Show "no" when exactly 0
            tooltipCounter.textContent = currentMessages === 0 ? 'no' : currentMessages;
        }
